...
### 변경
- 릴리스 트리거를 `v*` 태그 push → **master 로 PR 머지 시**로 변경.
  `FyneApp.toml`의 `Version`으로 태그를 만들고, 같은 태그가 없을 때(= 버전을 올린 PR)만 릴리스합니다.
...
- **말투(스타일) 선택** — 상단에서 `슨체`/`누체`를 고를 수 있습니다.
  ...
  - `누체`: 마지막 어절의 어미를 분석해 어간에 `누~`를 붙임 (예: `안녕하세요` → `안녕하누~`, `감사합니다` → `감사하누~`).
    - 사전 없이 "긴 어미부터 맞추는 접미사 테이블 + 받침 융합 어미(ㅂ니다/ㄴ다) 자모 처리"로 구현.
  - 스타일을 바꾸면 현재 입력이 즉시 다시 번역됩니다.
