...
문장 끝을 재밌게 바꿔주는 말투입니다. 문장부호는 그대로 유지돼요.
상단 `말투`에서 스타일을 고를 수 있슨:

- **슨체**(기본): 문장 마지막 **음절을 `슨`으로 교체** (한슨어 고유 개그)
- **누체**: 마지막 어절의 **어미를 분석해 어간에 `누~`** (디시 "~노/누" 말투)

| 한국어 | 슨체 | 누체 |
|--------|--------|--------|
| 안녕하세요 | 안녕하세**슨** | 안녕하**누~** |
| 감사합니다 | 감사합니**슨** | 감사하**누~** |
| 존나 웃기네 | 존나 웃기**슨** | 존나 웃기**누~** |
| 과자 | 과**슨** | 과**누~** |
| 아웃겨! | 아웃**슨**! | 아웃**누~**! |
| 밥 먹었어요? | 밥 먹었어**슨**? | 밥 먹었**누~**? |
...
말투는 `Style` 로 나뉩니다 — `슨체`는 `slap()`(마지막 음절 `슨` 교체), `누체`는 `nuWord()`(어미 분석 후 어간 `+누~`). 규칙을 바꾼 뒤 `go test ./...`로 검증하세요.
